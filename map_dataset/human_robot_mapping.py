...
rh_rf_palm = 95
rh_lf_palm = 86.6
rh_palm = np.array([rh_tf_palm, rh_ff_palm, rh_mf_palm, rh_rf_palm, rh_rf_palm])
...
for ln in label_source:
    ll = ln[0:63]
    label.append([float(l.replace(" ", "")) for l in ll])

label = np.array(label)
DataFile.close()

def HumanHandLoader(start, batch_size):
    keys = []
    local_pp = []
    shadow_pp = []
    # the joint order is
    ...
    for index in range(start, start + batch_size):
        ...
        hh_palm0 = np.linalg.norm(palm, axis=1)
...
if __name__ == '__main__':
    ...
    batch_size = 5
    # enables matplotlibinteractive mode

    for i in range(5,len(framelist), batch_size):
        keys, local_p, shadow_p = HumanHandLoader(i, batch_size)
